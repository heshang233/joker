package com.resteasy.netty.server;
...
public class ConfigurableNettyJaxrsServer extends NettyJaxrsServer {
	...
	public void start()
	{
		...
		if (sslContext == null) {
			factory = new HttpServerPipelineFactory(dispatcher, root, executorThreadCount, maxRequestSize, true, new ArrayList<>());
		} else {
            factory = new HttpsServerPipelineFactory(dispatcher, root, executorThreadCount, maxRequestSize, true, new ArrayList<>(), sslContext);
        }
		// Set up the event pipeline factory.
		bootstrap.setPipelineFactory(factory);

		// Bind and start to accept incoming connections.
		channel = bootstrap.bind(new InetSocketAddress(configuredPort));
